...
<tr>
    <td colspan="2" style="width:800px;paddin-left:20px;"><center><h4>LAPORAN STOK BARANG PER-KATEGORI</h4></center><br/></td>
</tr>
...
<?php 
    ...
    if($group=='-' || $group!=$d['kategori_nama']){
        $kat=$d['kategori_nama'];
        $query=$this->db->query("SELECT kategori_id,kategori_nama,bnama,SUM(bstok) AS tot_stok FROM kategori JOIN barang ON kategori_id=bkategori_id WHERE kategori_nama='$kat'");
        $t=$query->row_array();
        $tots=$t['tot_stok'];
        if($group!='-')
        echo "</table><br>";
...
$nomor=1;
    ...
    $group=$d['kategori_nama'];
        ...
        <tr>
                ...
                <td style="vertical-align:top;padding-left:5px;"><?php echo $d['bnama']; ?></td>
                <td style="vertical-align:top;text-align:center;"><?php echo $d['bstok']; ?></td>  
        </tr>
